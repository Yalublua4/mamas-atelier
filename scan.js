...
/**
 * Получение точного времени добавления (создания) файла
 */
function getFileTime(fullPath) {
  try {
    const stats = fs.statSync(fullPath);
    // birthtimeMs — дата фактического создания/добавления файла в файловую систему
    // mtimeMs — дата изменения (запасной вариант, если система не вернула birthtime)
    return (stats.birthtimeMs && stats.birthtimeMs > 0) ? stats.birthtimeMs : stats.mtimeMs;
  } catch (err) {
    return 0;
  }
}
  ...
  for (const item of items) {
    ...
    if (item.isDirectory()) {
      ...
      if (ALLOWED_EXTENSIONS.has(ext)) {
        ...
        // Сохраняем путь и время создания
        results.push({
          webPath: webPath,
          time: getFileTime(fullPath)
        });
      }
    }
  }
...
function updateCatalog() {
  ...
  // Получаем файлы со всех подпапок вместе с метками времени
  const rawFiles = scanDirectory(IMAGES_DIR);

  // Сортируем: то, что было добавлено первее — оказывается в начале (от старых к новым)
  rawFiles.sort((a, b) => {
    if (a.time !== b.time) {
      return a.time - b.time;
    }
    return a.webPath.localeCompare(b.webPath, 'ru');
  });

  // Получаем массив путей для сохранения в catalog.json
  const imageFiles = rawFiles.map(f => f.webPath);
  ...
  console.log(`⏱️ Файлы отсортированы по дате добавления (первые добавленные — вверху списка)`);
